...
%         for tt = 3 : nt_step
%             
%             u(:,tt)= dt^2*(M_inv*(-1*K*u(:,tt-1)))+2*u(:,tt-1)-u(:,tt-2);
%             u(n_e+1,tt)=0;
%             u(1,tt)=0;
%             
%         end
%         
%     elseif use_input_acc == 1
%         
%         for tt = 3 : nt_step
%             
%             u(:,tt)= dt^2*(M_inv*(-1*K*u(:,tt-1)))+2*u(:,tt-1)-u(:,tt-2);
...
%     end
%     
% elseif use_damping == 1 || use_damping == 2 || use_damping == 4
    
%     hWaitBar = waitbar(0, 'Solving in the time domain ...'); 
...
%             a3= M_inv*a1;
%        
%             u(:,tt)= dt^2*(a3-a2)+2*u(:,tt-1)-u(:,tt-2);
% %                    u(n_e+1,tt)=0;
% %               u(1,tt)=0;
% 
% %         waitbar(tt/nt_step, hWaitBar) 
%     end
